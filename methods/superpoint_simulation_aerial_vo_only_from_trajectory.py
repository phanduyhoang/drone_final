...
# Ensure project root (one level up) is on sys.path so 'magicpoint' and 'metrics' are importable
...
def main():
    ...
    # cam_sign override: use --cam-sign 1 to force correct convention and skip calibration
    # This is CRITICAL for images with low texture (snow, uniform areas) where auto-calibration fails.
    ap.add_argument("--cam-sign", type=int, default=None, choices=[1, -1],
                    help="Force cam_sign (+1 or -1) and skip auto-calibration. "
                         "Use --cam-sign 1 --yaw-use prev for reliable results on any image.")

    # trajectory centering: shift trajectory center to a fraction of image size
    # default None means use the trajectory's own coordinates (auto-scaled to image)
    ap.add_argument("--cx", type=float, default=None,
                    help="Trajectory center X as fraction of image width (e.g. 0.5 = center). "
                         "Use to move trajectory away from no-data borders.")
    ap.add_argument("--cy", type=float, default=None,
                    help="Trajectory center Y as fraction of image height (e.g. 0.5 = center).")
    ...
    ap.add_argument("--save-metrics", type=str, default=None,
                    help="Path to save JSON metrics (e.g. results/terrain_vo_metrics.json)")
    ...
    # Optional: re-center trajectory to a specific image fraction
    # This is useful for images where the default center is on a no-data border.
    if args.cx is not None or args.cy is not None:
        xs = [t[0] for t in traj_xy_yaw]
        ys = [t[1] for t in traj_xy_yaw]
        traj_cx = (min(xs) + max(xs)) * 0.5
        traj_cy = (min(ys) + max(ys)) * 0.5
        target_cx = (args.cx if args.cx is not None else 0.5) * W
        target_cy = (args.cy if args.cy is not None else 0.5) * H
        dx_shift = target_cx - traj_cx
        dy_shift = target_cy - traj_cy
        traj_xy_yaw = [(x + dx_shift, y + dy_shift, yaw) for (x, y, yaw) in traj_xy_yaw]
        print(f"[SHIFT] trajectory recentered by ({dx_shift:.1f}, {dy_shift:.1f}) px -> center ({target_cx:.0f}, {target_cy:.0f})")

    # Determine cam_sign and yaw_use
    # If --cam-sign is provided, skip auto-calibration (critical for low-texture images).
    # If only --yaw-use is provided, still run calibration for cam_sign.
    yaw_sign = float(args.yaw_sign)
    if args.cam_sign is not None:
        cam_sign = int(args.cam_sign)
        yaw_use_auto = "prev"  # safe default when skipping calibration
        print(f"[FORCED] cam_sign={cam_sign} (skipping auto-calibration)")
    else:
    ...
    print("=== Running VO (SuperPoint+SuperGlue) — NO LOOP CLOSURE ===")
